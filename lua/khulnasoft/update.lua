...
function M.validate(callback)
	...
	io.job({
		...
		on_exit = function(self, _)
			...
			notify.warn(
				"Khulnasoft.nvim: the version of the Khulnasoft server could not be determined, make sure it matches "
				.. versions.extension
			)
			callback(nil)
		end,
...
function M.download(callback)
	...
	local function hint(err)
		notify.info(
			"Khulnasoft.nvim: you can manually install the server",
	...
	local function chmod()
		io.set_executable(info.bin, function(_, err)
			if err then
				notify.error("Khulnasoft.nvim: failed to chmod server", err)
				hint("chmod_failed")
			else
				notify.info("Khulnasoft.nvim: server updated")
				M.validate(callback)
			end
		end)
	end
	...
	local function unpack()
		...
		io.gunzip(gz, function(_, err)
			if err then
				notify.error("Khulnasoft.nvim: failed to unpack server")
				hint("unpack_failed")
			else
				notify.info("Khulnasoft.nvim: server unpacked")
				chmod()
			end
		end)
	end
	...
	local function download()
		...
		io.download(info.download_url, gz, function(_, err)
			if err then
				notify.error("Khulnasoft.nvim: failed to download server", err)
				hint("download_failed")
			else
				notify.info("Khulnasoft.nvim: server downloaded")
				unpack()
			end
		end)
	end
