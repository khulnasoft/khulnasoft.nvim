...
local function khulnasoft_to_cmp(comp, offset, right)
	...
	local range = {
		start = {
			-- Khulnasoft returns an empty row for the first line
	...
	return {
		...
		cmp = {
			kind_text = "Khulnasoft",
			kind_hl_group = "CmpItemKindKhulnasoft",
		},
		khulnasoft_completion_id = comp.completion.completionId,
	}
end
...
if imported_cmp then
	cmp.event:on("confirm_done", function(event)
		if
			...
			and event.entry.source.name == "khulnasoft"
			and event.entry.completion_item
			and event.entry.completion_item.khulnasoft_completion_id
			and event.entry.source.source
			and event.entry.source.source.server
		then
			event.entry.source.source.server.accept_completion(event.entry.completion_item.khulnasoft_completion_id)
		end
	end)
end
	...
	local function handle_completions(completion_items)
		...
		for _, comp in ipairs(completion_items) do
			if not duplicates[comp.completion.text] then
				...
				table.insert(completions, khulnasoft_to_cmp(comp, offset, after_line))
			end
		end
