local versions = require("khulnasoft.versions")
local config = require("khulnasoft.config")
local io = require("khulnasoft.io")
local log = require("khulnasoft.log")
...
function Server.load_api_key()
	...
	if err or type(json) ~= "table" then
		if err == "ENOENT" then
			...
			local message = "please log in with :Khulnasoft Auth"
...
function Server.authenticate()
	...
	local function on_submit(value)
		...
		local endpoint = "https://api.khulnasoft.com/register_user/"
	...
	prompt = function()
		require("khulnasoft.views.auth-menu")(url, on_submit)
	end
...
function Server:new()
	...
	function m.start()
		...
		if not manager_dir then
			manager_dir = io.tempdir("khulnasoft/manager")
		...
		local function on_exit(_, err)
			...
			if err then
				...
				notify.error("khulnasoft server crashed", err)
	...
	function m.request_completion(document, editor_options, other_documents, callback)
		...
		request("GetCompletions", {
			...
			if err then
				...
				if ok and json then
					if json.state and json.state.state == "KHULNASOFT_STATE_INACTIVE" then
	...
	function m.get_chat_ports()
		request("GetProcesses", {
			...
			local url = "http://127.0.0.1:"
				...
				.. "&ide_version="
				.. versions.nvim
				.. "&app_name=khulnasoft.nvim"
				.. "&extension_name=khulnasoft.nvim"
				.. "&extension_version="
				.. versions.extension
