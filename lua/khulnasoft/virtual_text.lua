...
local enums = require("khulnasoft.enums")
local notify = require("khulnasoft.notify")
local util = require("khulnasoft.util")

local M = {}

local hlgroup = "KhulnasoftSuggestion"
local request_nonce = 0
local using_status_line = false

--- @type "idle" | "waiting" | "completions"
local khulnasoft_status = "idle"
...
function M.setup(_server)
	...
	local augroup = vim.api.nvim_create_augroup("khulnasoft_virtual_text", { clear = true })
...
local function completion_inserter(current_completion, insert_text)
	...
	local insert_text = '<C-R><C-O>=v:lua.require("khulnasoft.virtual_text").get_completion_text()<CR>'
...
local function clear_completion()
	local namespace = vim.api.nvim_create_namespace("khulnasoft")
...
local function render_current_completion()
	...
	for idx, part in ipairs(parts) do
		...
		vim.api.nvim_buf_set_extmark(0, vim.api.nvim_create_namespace("khulnasoft"), row - 1, 0, data)
...
function M.complete(opts)
	...
	local cancel = server.request_completion(
		...
		function(success, json)
			...
			if json and json.state and json.state.state == "KHULNASOFT_STATE_SUCCESS" and json.completionItems then
...
function M.status()
	if khulnasoft_status == "completions" then
		...
		return { state = khulnasoft_status }
	end
end
