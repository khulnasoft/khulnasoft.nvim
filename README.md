...
![License](https://img.shields.io/github/license/KhulnaSoft/khulnasoft.nvim)
...
[![built with Khulnasoft](https://khulnasoft.com/badges/main)](https://khulnasoft.com?repo_name=khulnasoft%2Fkhulnasoft.nvim)
...
Native [Khulnasoft](https://www.khulnasoft.com/) plugin for Neovim.
...
Khulnasoft. This can be done by running `:Khulnasoft Auth`, copying the token from
your browser and pasting it into API token request.

To use Khulnasoft Chat, execute the `:Khulnasoft Chat` command. The chat will be opened
...
- `api`: information about the API server to use:
  - `host`: the hostname. Example: `"khulnasoft.example.com"`. Required when using enterprise mode
...
- `wrapper`: the path to a wrapper script/binary that is used to execute any
  ...
  a FHS wrapper can be used for the downloaded khulnasoft server.
...
```lua
require('khulnasoft.virtual_text').setup({
    virtual_text = {
        filetypes = { 
            python = true,
            markdown = false
...
When using virtual text, Khulnasoft status can be generated by calling `require('khulnasoft.virtual_text').status_string()`. 
...
```lua
require('khulnasoft.virtual_text').set_statusbar_refresh(function()
...
`error` by exporting the `DEBUG_KHULNASOFT` environment variable.
